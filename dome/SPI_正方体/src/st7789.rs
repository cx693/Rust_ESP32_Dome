...
/// 画水平线 (内部辅助)
fn draw_hline(fb: &mut [u8], x0: i16, y: i16, x1: i16, hi: u8, lo: u8, w: usize) {
    if y < 0 || y >= HEIGHT as i16 { return; }
    let row = y as usize * w;
    let x0 = x0.max(0) as usize;
    let x1 = (x1 as usize).min(WIDTH as usize - 1);
    for x in x0..=x1 {
        let idx = (row + x) * 2;
        fb[idx] = hi;
        fb[idx + 1] = lo;
